...
export class DashboardController {
  overview = async (_req: AuthRequest, res: Response) => {
    try {
      const [
        clientStats, projectStats, invoiceStats, workerStats,
        recentProjects, recentInvoices, monthlyRevenue, taskOverview
      ] = await Promise.all([
        AppDataSource.query("SELECT COUNT(*) as total, SUM(isActive=1) as active FROM clients"),
        AppDataSource.query(`
      ...
      const [summary, byWorker, byProject] = await Promise.all([
        AppDataSource.query(
          `SELECT COALESCE(SUM(hoursWorked),0) as totalHours,
           ...
           FROM worker_logs wl WHERE ${clause} AND status != 'rejected'`, params
        ),
        AppDataSource.query(
          `SELECT w.id, w.firstName, w.lastName, w.role,
           ...
           FROM worker_logs wl JOIN workers w ON w.id = wl.workerId
